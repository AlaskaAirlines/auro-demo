	...
	return {
		...
		buildStart() {
			...
			const sourceDir = path.resolve(
				"./node_modules/@aurodesignsystem/webcorestylesheets/dist/bundled/themes",
			);
			...
			const themesToCopy = [
				...
				{
					source: "alaska-classic.global.min.css",
					dest: "alaska-classic.global.min.css",
				},
			...
			themesToCopy.forEach((theme) => {
				...
				try {
					...
					if (fs.existsSync(sourcePath)) {
						...
						console.log(
							`Successfully copied: ${theme.source} -> ${theme.dest}`,
						);
					} else {
